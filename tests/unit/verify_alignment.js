...
T('release date comes from the localization changelog module', () => {
  const runtimeDate = changelogSource.match(
    /export const CHANGELOG = \[\s*\{\s*version:\s*'v\d+\.\d+\.\d+',\s*date:\s*'(\d{4}-\d{2}-\d{2})'/,
  )?.[1];
