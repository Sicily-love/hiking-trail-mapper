...
- `features/`：单一功能 controller；轨迹变更、IndexedDB、文件导入、手动标注点和测距会话状态已经迁入 typed controller，其余 feature runtime owner 按同一方式逐步迁移。
...
`runtime.ts` 已达到 400 行最终护栏，只保留启动和命令胶水。新增行为不得写回模板，应进入对应 typed controller。垂直文件拆分已经完成，trail、storage、file import、waypoint 与 measure 已迁入 typed controller；测距的 Leaflet 图层和帧句柄仍由 runtime adapter 持有。
...
- `features/`: single-feature controllers; trail mutations, IndexedDB, file import, manual-waypoint state/writes, and measurement session state now live in typed controllers, while the remaining feature owners migrate through the same pattern.
...
`runtime.ts` now meets the final 400-line guardrail and contains only boot and command glue. New behavior must not return to the template; place it in a typed controller. The vertical file split is complete, and trail, storage, file import, waypoint, and measure now use typed controllers; Leaflet layers and frame handles for measurement remain in the runtime adapter.
