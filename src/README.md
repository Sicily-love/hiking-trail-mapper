...
- `features/`：单一功能 controller；轨迹变更、IndexedDB、文件导入和手动标注点状态/写入已经迁入 typed controller，其余 feature runtime owner 按同一方式逐步迁移。
...
`runtime.ts` 已达到 400 行最终护栏，只保留启动和命令胶水。新增行为不得写回模板，应进入对应 typed controller。垂直文件拆分已经完成，trail、storage、file import 与 waypoint 已按 `RuntimeContext` 样板迁移，其余 classic owner 仍需逐步强类型化。
...
- `features/`: single-feature controllers; trail mutations, IndexedDB, file import, and manual-waypoint state/writes now live in typed controllers, while the remaining feature owners migrate through the same pattern.
...
`runtime.ts` now meets the final 400-line guardrail and contains only boot and command glue. New behavior must not return to the template; place it in a typed controller. The vertical file split is complete, trail, storage, file import, and waypoint follow the `RuntimeContext` template, and the remaining classic owners still need gradual typing.
