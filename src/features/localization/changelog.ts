import type { ContentDialogOptions } from '../../ui/dialog/content-model.ts';
import type { LocalizationLanguage } from './translations.ts';

export interface ChangelogEntry {
  version: string;
  date: string;
  items: Record<LocalizationLanguage, string[]>;
}
...
export const CHANGELOG = [
  {
    ...
    date: '2026-08-20',
    items: {
      zh: ['大项目地图按设备能力自适应抽稀，仅减少显示点，原始轨迹、吸附、统计和导出保持全量精度。', 'Leaflet 轨迹与高点图层按 key 复用并显式释放，21.6 万点压力场景稳定堆降至约 43 MiB。', '地图标注按缩放级别控制密度并进行屏幕碰撞避让，主轨迹层级更清晰。', '完整项目备份改为兼容的紧凑 JSON，并减少校验和恢复期间的临时内存与文件体积。'],
      en: ['Adapt map display sampling to device capacity while preserving full source fidelity for snapping, statistics, and export.', 'Reuse and explicitly release keyed Leaflet trail and high-point layers, reducing stable heap to about 43 MiB in the 216k-point stress case.', 'Control map-label density by zoom and avoid screen-space collisions while strengthening primary-trail hierarchy.', 'Use compatible compact JSON project archives and reduce temporary memory and file size during validation and restore.'],
    },
  },
  {
    version: 'v3.0.1',
  ...
  },
  {
    ...
    date: '2026-08-15',
    items: {
      zh: ['启用全新 Workbench 3.0，以地图为中心重构应用栏、活动导航、路线资料库与海拔分析区。', '统一桌面、平板与手机布局，改进安全边距、触摸目标、侧栏底部面板和响应式反馈。', '修正海拔/标注模式入口重叠，并将测距重选、反向和退出全部接入统一命令。', '精简轨迹导入窗口与顶部操作分隔，保持 v2.3 项目备份、轨迹数据和 Leaflet 渲染完全兼容。'],
      en: ['Launch Workbench 3.0 with a map-first app bar, activity navigation, route library, and elevation analysis dock.', 'Unify desktop, tablet, and phone layouts with safer insets, touch targets, mobile sheets, and responsive feedback.', 'Fix overlapping elevation and waypoint mode controls, and route measure reset, reverse, and exit through semantic commands.', 'Simplify the route import surface and toolbar grouping while preserving v2.3 project archives, trail data, and Leaflet rendering.'],
    },
  },
  {
    version: 'v2.3.5',
    date: '2026-08-14',
    items: {
      zh: ['拆分渲染、命令、生命周期和存储运行时所有权，进一步收紧状态与 DOM 边界', '优化移动端地图提示与标注卡片安全区域，并强化大型项目归档、缓存恢复和连续编辑测试'],
      en: ['Split render, command, lifecycle, and storage runtime ownership to tighten state and DOM boundaries', 'Improve mobile map overlay safety and strengthen large-project archive, cache restore, and continuous-edit tests'],
    },
  },
  {
    version: 'v2.3.4',
  ...
  },
  {
    ...
    date: '2026-08-11',
    items: {
      zh: ['重构测距与行程分段运行时：DOM、Leaflet 图层和拖动调度迁入独立 owner，主 runtime 缩减至约 1560 行', '新增共享轨迹吸附与地图输入模块，统一快速点击、邻近点缓存和拖动帧合并', '统一分段面板的桌面与移动端布局，移除重复的末日撤销按钮并保留按日删除与还原'],
      en: ['Extracted measurement and itinerary segmentation into dedicated runtime owners, moving DOM, Leaflet layers, and drag scheduling out of the main runtime', 'Added shared trail snapping and map input modules for cached nearest-point lookup, fast taps, and coalesced drag frames', 'Unified the segment panel across desktop and mobile, removing the redundant last-day undo action while retaining per-day delete and restore'],
    },
  },
  {
    version: 'v2.3.2',
    date: '2026-08-11',
    items: {
      zh: ['将下撤规划的交互、Leaflet 图层与面板 DOM 迁入 typed escape runtime owner，主 runtime 缩减至约 2450 行，并移除业务状态中的渲染句柄。', '优化下撤与分段面板的信息层级、A/B 状态反馈、窄屏高度和触摸目标，并新增真实下撤规划截图回归。'],
      en: ['Move escape planning interactions, Leaflet layers, and panel DOM into a typed escape runtime owner, reduce the main runtime to about 2,450 lines, and remove rendering handles from business state.', 'Refine escape and segment panel hierarchy, A/B feedback, narrow-screen sizing, and touch targets, with a real escape-planning visual regression.'],
    },
  },
  {
    version: 'v2.3.1',
  ...
  },
  {
    ...
    date: '2026-08-10',
    items: {
      zh: ['完整项目恢复新增结构化预检、分阶段反馈与等待保存及地图复位完成的原子流程，并以 18 万轨迹点压力样例覆盖旧归档迁移和派生数据重建。', 'GitHub Pages 新增可安装 PWA 与应用外壳离线启动，保留单 HTML 发布，并明确卫星底图仍需网络。', '导出菜单迁入 typed UI owner，主 runtime 收缩至约 2960 行；新增 360×800 移动端与恢复预检视觉回归。'],
      en: ['Add structured project-restore preflight, staged feedback, and an atomic flow that waits for persistence and map reset, backed by a 180,000-point archive migration and derived-data stress regression.', 'Ship GitHub Pages as an installable PWA with offline application-shell startup while preserving the standalone single-HTML release; satellite imagery still requires a network.', 'Move the export menu into a typed UI owner, reduce the main runtime to about 2,960 lines, and add 360×800 mobile plus restore-preflight visual regressions.'],
    },
  },
  {
    version: 'v2.2.10',
  ...
  },
  {
    ...
    date: '2026-07-29',
    items: {
      zh: ['修复单文件发布中 ZIP 工作线程不兼容导致的轨迹组导出失败，并在压缩失败时自动降级下载 KML。', '提高项目备份的安全节点容量以支持大型密集多轨迹项目，同时保留文件大小、嵌套深度和数据有效性保护。'],
      en: ['Fix group export failures caused by ZIP worker incompatibility in the single-file release and automatically fall back to KML downloads when compression fails.', 'Increase the safe project-archive node capacity for large dense multi-trail projects while preserving byte-size, nesting-depth, and data-validity protections.'],
    },
  },
  {
    version: 'v2.2.8',
  ...
  },
  {
    ...
    date: '2026-07-22',
    items: {
      zh: ['撤销历史改用紧凑内部快照，并增加 30 条记录与 32 MiB 总预算，降低大轨迹和图片项目的内存占用。', '超大编辑无法安全进入历史时清空旧撤销栈并明确提示，避免撤销跨越当前项目状态。'],
      en: ['Stored undo history as compact internal snapshots with both a 30-entry and 32 MiB budget to reduce memory use on large routes and image-heavy projects.', 'Oversized edits now clear stale undo history and report that the edit could not be retained safely.'],
    },
  },
  {
    version: 'v2.2.0',
    date: '2026-07-21',
    items: {
      zh: ['项目归档升级至 schema 2，旧 schema 1 备份可自动迁移，恢复失败时自动回滚。', '新增项目级撤销与重做，覆盖轨迹、行程分段、标注、下撤与拼接等持久化编辑。', '继续拆分项目归档、恢复 UI 与历史编排，runtime 仅保留浏览器接线。', '优化触摸拖点命中、点击判定和复位动画，降低移动端误触与大跨度复位卡顿。'],
      en: ['Upgraded project archives to schema 2 with automatic schema 1 migration and rollback after failed restores.', 'Added project-level undo and redo for durable trail, itinerary, waypoint, escape-route, and stitching edits.', 'Moved archive, restore UI, and history orchestration out of runtime into typed project features.', 'Improved touch hit targets, tap detection, and reset transitions to reduce mobile misfires and expensive reset animation.'],
    },
  },
  {
    version: 'v2.1.0',
  ...
  },
  {
    ...
    date: '2026-07-18',
    items: {
      zh: ['每张轨迹卡新增统一的重命名按钮；主轨迹摘要也改用 typed TrailController 修改名称，并同步拼接来源和下撤依据轨迹中的持久化名称。', '桌面边栏恢复为 310px 固定宽度，主轨迹标题和轨迹卡长名称统一使用单行省略，避免名称撑宽工作区。', '提高底部提示条文字与背景的对比度，并根据地图与海拔面板的实际位置动态避让，避免提示被分析区遮挡。'],
      en: ['Add a consistent rename action to every trail card; primary-trail renaming now uses the typed TrailController and updates persisted stitch-source and escape-reference names.', 'Restore the desktop sidebar to a fixed 310px width and ellipsize long primary and trail-card names on one line so names cannot widen the workspace.', 'Increase bottom-toast text contrast and position feedback from the live map and elevation-panel bounds so the analysis dock cannot obscure it.'],
    },
  },
  {
    version: 'v2.0.15',
    date: '2026-07-18',
    items: {
      zh: ['轨迹拼接端点支持自由跟手拖动，拖动中显示吸附目标与引导线，松手后再吸附到来源轨迹。', '拼接片段使用编号端点、重合端点错位显示和当前路线强高亮，提升多路线编辑辨识度。', '测距操作栏可在海拔面板内拖动并记忆位置；选定拼接来源后等待布局稳定，再自动复位到全部来源轨迹。'],
      en: ['Stitch endpoints now follow the pointer freely, preview the nearest snap target, and commit to the source trail on release.', 'Numbered endpoints, collision offsets, and stronger active-route highlighting make multi-route stitching easier to distinguish.', 'The measurement action bar is draggable within the elevation dock and remembers its position; choosing stitch sources now waits for layout stability before fitting every source trail.'],
    },
  },
  {
    version: 'v2.0.14',
  ...
  },
  {
    ...
    date: '2026-07-14',
    items: {
      zh: ['删除 executeClassicScript、runtime composer、raw runtime import 和 13 个 classic owner，生产启动链改为直接 TypeScript 模块', '将版本真源迁移到 src/app/version.ts，第三方库纳入 Vite 模块图，并保留自包含单 HTML 与 GitHub Pages 发布', '重构测试为 direct runtime 契约，并通过仅测试模式 inspector 保持真实 Chrome、E2E 与视觉回归覆盖', '修复 Workbench 顶部七个菜单无法接收指针点击的问题，并统一菜单、活动栏、分析 Tab 和辅助控件的中英文切换'],
      en: ['Removed executeClassicScript, the runtime composer, raw runtime imports, and all 13 classic owners; production now boots through a direct TypeScript module', 'Moved version ownership to src/app/version.ts, loaded vendors through the Vite module graph, and retained the self-contained single-HTML and GitHub Pages release', 'Reworked tests around direct-runtime contracts and preserved real-Chrome, E2E, and visual coverage through a test-only inspector', 'Fixed pointer interaction across all seven Workbench menus and unified Chinese/English switching for menus, activity rail, analysis tabs, and auxiliary controls'],
    },
  },
  {
    version: 'v1.32.2',
    date: '2026-07-11',
    items: {
      zh: ['修复打开 HTML 从浏览器缓存恢复轨迹后未可靠自动复位的问题：清理失效缓存引用，等待地图布局完成后仅执行一次主轨迹复位'],
      en: ['Fixed unreliable automatic reset after restoring trails from browser cache when opening the HTML by cleaning stale cache references and fitting the primary trail exactly once after map layout'],
    },
  },
  {
    version: 'v1.32.1',
...
export function buildChangelogDialogModel(
  language: LocalizationLanguage,
  title: string,
  closeLabel: string,
): ContentDialogOptions {
  return {
    title, closeLabel, size: 'wide',
    sections: CHANGELOG.map(entry => ({
      heading: entry.version, meta: entry.date, items: entry.items[language] || entry.items.zh,
    })),
  };
}
