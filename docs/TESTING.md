...
自动化覆盖 390×844、360×800、320×568 和横屏视口，但不能替代 vivo S30 mini 等真实设备。发布冻结前按以下顺序验收一次：

1. 用 Chrome 打开 Pages 和下载后的单 HTML；确认缓存恢复后只复位一次，侧栏、底栏、海拔坞不遮挡地图。
2. 导入约 5 万、10 万、20 万点项目；切换轨迹组、主轨迹、海拔/标注模式，连续缩放与拖动均不出现白屏或浏览器退出。
3. 完成测距 A/B 拖动、Day 预览、分段应用/还原、标注新增、下撤和部分轨迹拼接。
4. 导出 KML、行程、ZIP 和完整项目备份，再从完整备份恢复；Day、标注图片、下撤方案和工作区选择应一致。
5. 首次在线打开后切换飞行模式，确认应用壳和本地数据可用。卫星底图仍依赖网络，不把第三方瓦片缓存视为离线地图保证。

验收记录至少包含设备、Android/Chrome 版本、项目点数、是否通过和异常截图。自动测试只报告模拟视口通过，不能写成“已在 vivo 真机验证”。
