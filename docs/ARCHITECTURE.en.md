...
│       ├── context.ts
│       └── studio.ts             Shared map orchestration (~1,560 lines)
...
`features/waypoint/runtime-owner.ts` owns the waypoint editor, image reads, right-click/long-press gestures, and unified interaction session. `features/map/runtime-owner.ts` composes typed render models, Leaflet adapters, overlay UI, and track-point inspection for map tracks and markers. `ui/` controllers own the Lightbox zoom/drag/touch lifecycle and the collapsed-sidebar primary card. `studio.ts` no longer retains these listeners, timers, layer-diff updates, or position state.

### Direct Runtime

`src/app/runtime/studio.ts` has been reduced from roughly 6,200 to roughly 1,370 lines. KML project building, reset/fit ownership, map rendering, sidebar/itinerary DOM, import DOM, trail stitching, measurement, segmentation, waypoint interaction, escape planning, and elevation Canvas now have dedicated controllers or owners; shared trail snapping and map input also have dedicated modules. All TypeScript source passes strict checking without `@ts-nocheck`. The main runtime composes stores, actions, selectors, commands, and browser capabilities without retaining measurement/segment DOM or Leaflet rendering implementations. Production publishes no business globals, while `?studio-test=1` creates a deeply read-only test inspector and a dedicated fixture driver.
